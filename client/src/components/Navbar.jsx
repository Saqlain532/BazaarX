  ...
  return (
    <nav className="flex items-center justify-between px-6   md:px-16 lg:px-24 xl:px-10 py-4 border-b border-gray-300 bg-white text-black transition-all">
            ...
            <NavLink to='/' onClick={()=>setOpen(false)}>
                   ...
                   <BazaarXLogo height={30} className='ml-0'/>
            </NavLink>

            {/* Desktop Menu */}
            <div className="hidden sm:flex items-center gap-8 text-cyan-700">
                ...
                (
                    <div className='relative group bg-primary/10' >
                        <img src={assets.profile_icon} className='w-10' alt="profile" />
                        <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadow border text-shadow-primary-dull border-gray-200 py-2.5 w-30 rounded-md text-sm z-40' >
                            <li  onClick={ ()=>{
                                navigate("my-orders")
            ...
            <div className={`${open ? 'flex' : 'hidden'} absolute top-15 left-0 w-full bg-primary/10 shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
...

               {
                !user ? (
                    ...
                    className="text-white cursor-pointer px-6 py-2 mt-2 bg-primary hover:bg-primary-dull transition  rounded-full text-sm">
                    Logout
                </button>
                )
               }
