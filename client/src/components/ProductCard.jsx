...
const ProductCard = ({product}) => {
    ...
    return product && (
        ...
        className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white flex-col justify-between w-full">
            <div className="group cursor-pointer flex items-center justify-center px-2">
                <img
                    className="group-hover:scale-[1.05] transition max-w-26 md:max-w-36"
                    src={product.image[0]}
                    alt={product.name}
                />
            </div>
                ...
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div onClick={(e)=>{ e.stopPropagation();} } className="text-primary-500">
                        {!cartItems[product._id] ? (
                            <button
                                className="flex items-center justify-center cursor-pointer gap-1 bg-primary/10 border border-primary md:w-20 w-16 h-8.5 rounded text-primary font-medium"
